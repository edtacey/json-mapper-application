...
import { Save, Upload, Download, Code, Shield, Copy, FileText } from 'lucide-react';
import Editor from '@monaco-editor/react';
import toast from 'react-hot-toast';
import * as yaml from 'js-yaml';
import { api } from '../services/api';
import { AbstractedModelManager } from '../components/AbstractedModel';
import { AbstractedFlag } from '../components/entity/AbstractedFlag';
...
export const EntityEditor: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'sample' | 'api' | 'schema' | 'abstracted'>('sample');
  ...
  const updateSchemaMutation = useMutation(
      ...
      api.entities.updateSchemas(id!, data),
    {
      ...
      onError: () => {
        toast.error('Failed to update schemas');
      }
    }
  );
  ...
  const handleCloneToDestination = () => {
    ...
    toast.success('Source schema cloned to destination');
  };
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="p-6 border-b border-gray-200">
          ...
          <div className="mt-4">
            <AbstractedFlag
              checked={entityData.abstracted}
              onChange={(checked) => setEntityData({ ...entityData, abstracted: checked })}
              disabled={false}
            />
          </div>
        </div>
                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={toggleSchemaFormat}
                        className="inline-flex items-center px-3 py-1 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
                      >
                        <FileText className="mr-1 h-4 w-4" />
                        {schemaFormat === 'json' ? 'JSON' : 'YAML'}
                      </button>
                      <button
